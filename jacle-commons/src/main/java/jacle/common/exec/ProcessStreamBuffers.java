...
import jacle.common.thread.ExecutorsExt;
...
public class ProcessStreamBuffers implements Closeable {
    ...
    /**
     * <p>
     * Interrupts the stream readers/writers by closing the {@link InputStream}
     * s, and then waits for the background threads to terminate. A side effect
     * of this method is that the streams owned by the {@link Process} provided
     * to {@link #bind(Process)} will be closed.
     * </p>
     * 
     * <p>
     * After calling this method, you should still call {@link #waitFor()},
     * which will ensure that the background threads terminate and the internal
    ...
    /**
     * Waits for all stream copying to complete (along with the related
     * background threads). Calls {@link #close()} internally, so there's no
     * need to call that explicitly.
     */
     ...
     * important to call this method in order to avoid leaked resources. You do
     * not need to call this method explicitly if you call {@link #waitFor()}.
     */
    @Override
    public void close() {
